...
DATA_FOLDER="${BASE_FOLDER}/GDC"
SCRIPT_FOLDER="${BASE_FOLDER}/bin/GDC_scripts"
...
de_analysis( ) {
	...
	Rscript --vanilla ${SCRIPT_FOLDER}/gdc_tcga_analyse_stratify_edgeR.R ${GOI} ${working_folder} ${strat_by} ${percentile} ${REF_FILES_FOLDER} ${tcga_fpkm[0]} ${tcga_counts[0]} ${tcga_rpm[0]}
	cd ${working_folder}
}
...
USAGE: $(basename $0) [-h] -p <TCGAProjectID> -g <GOI> -s <StratifyBy> -t <Percentile> -a -c -d
	where:
...
if grep -Fxq "${project_id^^}" <(cut -f 1 ${SCRIPT_FOLDER}/GDC_API/TCGA_Study_Abbreviations.tsv)
then
	...
	echo ""
	cat ${SCRIPT_FOLDER}/GDC_API/TCGA_Study_Abbreviations.tsv
	exit 1
fi
...
### Sign on
echo "Starting Correlation and DE analysis for ${GOI} using ${project_id}"
