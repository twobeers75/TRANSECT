...
if (gene1 %in% colnames(df_gtex_data)) {
...
high_expr_df = data.frame(Names=row.names(df_gtex_data))
...
# for (row in 1:10) { 
  ...
  if (gene2 %in% colnames(df_gtex_data)) {
    gene2_exp_value <- as.numeric(unlist(df_gtex_data[,gene2]))
...
p <- ggplot(df_target_pairs, aes(x = logExp_Cor, y = -log10(logExp_FDR))) +
  ...
  geom_point(data = subset(df_target_pairs, logExp_FDR < 1E-150 & logExp_Cor > 0.8), shape=20, size=0.75, color="red") +
  geom_point(data = subset(df_target_pairs, logExp_FDR < 1E-150 & logExp_Cor < -0.8), shape=20, size=0.75, color="blue") +
