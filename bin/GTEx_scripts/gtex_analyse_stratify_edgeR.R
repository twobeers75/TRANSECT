...
###*****************************************************************************
gsea_exe <- "/data_b/tools/GSEA/GSEA_Linux_4.2.3/gsea-cli.sh GSEAPreranked"
...
if (length(args)==0) {
	...
	# default to
  args[2] = "/home/jtoubia/Desktop/Projects/SRt"
  args[3] = "mRNA"
  args[4] = 5
  args[5] = "/home/jtoubia/Desktop/Projects/SRt/REF_FILES"
	args[6] = list.files(args[2], "GTEx/GTEx-v8/Breast", "GTEx-Breast-*_tpm-mRNA.tsv", full.names=TRUE)
	args[7] = list.files(args[2], "GTEx/GTEx-v8/Breast", "GTEx-Breast-*_count-mRNA.tsv", full.names=TRUE)
	args[8] = ""
	arg2[9] = ""
}

GOI <- args[1]
outdir <- paste(args[2], "/Analyses/DE", sep="/")
rna_species <- args[3]
...
if (rna_species == "mRNA"){
  # exp_data <- read.delim(gene_fpkm_filename, sep='\t', check.names=FALSE, stringsAsFactors=FALSE, row.names = "Description")
  ...
  exp_data <- exp_data %>% column_to_rownames('Description')
  exp_type <- "TPM"
...
if (grepl( "+", GOI, fixed = TRUE)){
  ...
  GOI <- unlist(strsplit(GOI, "+", fixed=TRUE))
  if (length(GOI) > 5) {
...
if (multi_GOI_analysis){
  ...
  GOI_exp_sep_long <- gather(GOI_exp_sep, "geneID", "expression", all_of(GOI), factor_key=TRUE)
...
if (ratio_GOI_analysis){
  ...
  ### calc log2 FC
  GOI_exp_raw$ranking_score = (log2(GOI_exp_raw[,GOI[1]])) - (log2(GOI_exp_raw[,GOI[2]]))
  ### now divide raw expression values for plotting
  GOI_exp_raw$div <- GOI_exp_raw[,GOI[1]] / GOI_exp_raw[,GOI[2]]
  ### reset GOI variable
  GOI <- paste(GOI, collapse="%")
...
if (single_GOI_analysis){
  ...
  par(mar=c(5,5.5,4,2))
  hist(log2(GOI_exp[, GOI]), breaks=100, main=GOI, 
