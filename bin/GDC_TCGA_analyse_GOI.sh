...
# working_folder=`pwd`
# for now, lets assume that we will always get the full path, my scripts are custom built
...
de_analysis( ) {
	...
	Rscript --vanilla ${SCRIPT_FOLDER}/gdc_tcga_analyse_stratify_edgeR.R ${GOI} ${output_folder} ${strat_by} ${percentile} ${REF_FILES_FOLDER} ${GSEA_EXE} ${tcga_fpkm[0]} ${tcga_counts[0]} ${tcga_rpm[0]}
	return_code=$?
	if [[ $return_code -ne 0 ]]
	then
		return $return_code
...
USAGE: $(basename $0) [-h] -p <TCGAProjectID> -g <GOI> -s <StratifyBy> -t <Percentile> -a -c -d -o	where:
